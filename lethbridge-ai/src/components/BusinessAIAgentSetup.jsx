...
function BusinessAIAgentSetup({ businessId, currentConfig, onSave, onClose }) {
  ...
  const [config, setConfig] = useState({
    ...
    include_platform_data: currentConfig?.include_platform_data || false,
    ...
    menu_data: {
      categories: currentConfig?.menu_data?.categories || []
    },
    faq_data: Array.isArray(currentConfig?.faq_data) ? currentConfig.faq_data : [],
            ...
            <div className="space-y-6">
              <div>
                ...
                <label className="flex items-center mb-4">
                  <input
                    type="checkbox"
                    checked={config.include_platform_data}
                    onChange={(e) => setConfig({ ...config, include_platform_data: e.target.checked })}
                    className="rounded border-gray-300 text-purple-600 focus:ring-purple-600"
                  />
                  <span className="ml-2 text-sm font-medium text-gray-700">
                    Include Lethbridge events and business information
                  </span>
                </label>
                <p className="text-xs text-gray-500 ml-6 -mt-2">
                  Allow AI to answer questions about other businesses and events in Lethbridge while prioritizing your business
                </p>
              </div>
