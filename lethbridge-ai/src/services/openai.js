...
export async function enhanceDescription(originalText, type = 'event') {
  if (!originalText || originalText.trim().length < 10) {
    throw new Error('Please provide at least a brief description to enhance');
  }
  ...
  const prompts = {
    event: `You are a professional event copywriter for Lethbridge, Alberta. Take this basic event description and transform it into an engaging, compelling description that will attract attendees. Keep it concise (max 150 words), friendly, and highlight what makes this event special. Include a clear call-to-action. Original description: "${originalText}"`,
    
    listing: `You are helping create a community listing for Lethbridge. Take this basic description and make it more appealing and clear. Keep it friendly, concise (max 100 words), and highlight the value or unique aspects. Original description: "${originalText}"`,
    
    business: `You are a professional business event copywriter. Transform this description into compelling marketing copy that attracts customers. Be professional yet approachable, highlight key benefits, and include a subtle call-to-action. Keep it under 150 words. Original description: "${originalText}"`
  };
  ...
  try {
    const response = await callOpenAI([
      { 
        role: 'system', 
        content: prompts[type] || prompts.event 
      },
      { 
        role: 'user', 
        content: 'Generate the enhanced description. Return only the improved description text, no additional commentary.' 
      }
    ], {
      temperature: 0.7,
      max_tokens: 250
